...
public class Lc7 {
    ...
    static class BT_to_Array {
        ...
        public TreeNode formTree(int[] arr) {
            return helper(arr, 1);
        }

        private TreeNode helper(int[] arr, int pos) {
            if (pos < arr.length && arr[pos] != -1) {
                TreeNode root = new TreeNode(arr[pos]);
                root.left = helper(arr, pos * 2);
                root.right = helper(arr, pos * 2 + 1);
                return root;
            } else {
                return null;
            }
    ...
    public static void main(String[] args) {
        ...
        TreeNode treeNode = test.formTree(res);
        BT_to_Array.printTree(treeNode);
